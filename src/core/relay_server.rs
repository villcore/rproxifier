...
use crate::core::proxy_config_manager::{HostRouteStrategy, ProxyServerConfig, ProxyServerConfigType};
use crate::{ProxyServerConfigManager, ProcessManager};
    ...
    async fn accept_socket(&self, mut tcp_socket: TcpStream, socket_addr: SocketAddr) {
        ...
        match nat_session_manager.get_port_session_tuple(session_port) {
            ...
            Some((src_addr, src_port, dst_addr, dst_port)) => {
                ...
                let (rx_connection_manager, tx_connection_manager) = (active_connection_manager.clone(), active_connection_manager.clone());
                tokio::spawn(async move {
                    ...
                    if let Some(socket_info) = process_manager.get_process_by_port(src_port) {
                        ...
                        active_connection_manager.add_connection(ActiveConnection {
                            pid,
                            process_name: process.unwrap_or_else(||"".to_string()),
                            session_port,
